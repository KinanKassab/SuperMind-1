...
export class QuestionGenerator {
   ...
   * Fixed rule: factorA in [0..10], factorB in [10..100]
  ...
  generateQuestion(options = {}) {
    ...
    do {
      question = this.createQuestion({ difficulty });
      attempts++;
  ...
  /**
   * Create a single question using the fixed rule
   * @param {string} difficulty - Difficulty level
   * @returns {Object} Question object
   */
  createQuestion({ difficulty }) {
    const factorA = randomInt(0, 10);
    const factorB = randomInt(0, 100);
  ...
  // Removed old rule-based generation; only the fixed rule is supported now
